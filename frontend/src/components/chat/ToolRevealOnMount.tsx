...
export const ToolRevealOnMount: React.FC<ToolRevealOnMountProps> = ({
  ...
  React.useLayoutEffect(() => {
    ...
    const frame = window.requestAnimationFrame(() => {
      ...
      void animation.finished
        .catch(() => undefined)
        .finally(() => {
          clearRevealStyles(rootRef.current);
        });
    });
  ...
  const combinedClassName = [child.props.className, className].filter(Boolean).join(' ') || undefined;
...
export default ToolRevealOnMount;
