...
2026-05-02: Fixed two production blockers after modular API rollout: (1) `GET /api/git/worktrees/bootstrap-status` now returns the typed contract expected by frontend (`{ status, error, updatedAt }`) instead of a custom `{ bootstrapped }` payload, which stopped runaway polling loops and unblocked message send paths waiting on bootstrap readiness; (2) `prompt_async` now handles stale client model keys more safely by selecting a valid fallback model from `runner.listModels()` when the requested model is not found. Backend build/tests are green and service restarted.
