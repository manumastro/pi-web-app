...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, `/api/agent` retry storm, message-ID reconciliation + assistant stream IDs, part-first stream materialization, and now auto-creates a session on send when no active session is selected (no empty-sessionId sends).
