...
- Streaming/tooling hardening: runner-process now emits a fallback `done` when the blocking RPC prompt returns without an `agent_end`, preventing end-of-stream stuck UI; runner/orchestrator forwards status/usage metadata updates, assistant status surfaces token/context metadata when available, and consecutive tool bursts are collapsed into an expandable grouped tool card to avoid chat jank.
