...
2026-05-04: Fixed assistant text duplication root cause for snapshot-correction adapter events. Added orchestrator-level correction detection: incoming text that shares >50% prefix with already-emitted content (and is not a pure extension) is now emitted as `message.part.updated` (replacement) instead of `delta` (append), handling typo corrections and rephrasings from adapters that emit cumulative snapshots. Added `replace` flag to `text_chunk` SSE schema and event-mapper support, plus per-text-event diagnostic logs. Backend tests pass (119/119). Frontend rebuilt and service restarted.
