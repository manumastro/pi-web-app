...
Current state (2026-04-26): See `BLUEPRINT.md` (sections **15.0 Status Snapshot**, **5.1 Feature Matrix**, and phase checklists **15.1–15.6**) as the single up-to-date source of project status; latest significant change starts PizzaPi UX parity with a reliability-first frontend increment and `docs/UX_PARITY_ROADMAP.md`: SSE reconnects now preserve `lastEventId`, replayed events are deduplicated, adjacent text chunks are frame-coalesced, stale streams force reconnect, and sidebar session rows expose live working/retry/question/permission/error badges; frontend lint/tests are green with 75 tests.
