...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, message-ID reconciliation + assistant stream IDs, part-first stream materialization, empty-state sends, and now returns SDK-complete message/model metadata so OpenChamber does not fall back to Gemma/unknown `?` rendering.
