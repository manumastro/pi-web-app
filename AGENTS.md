...
Current state (2026-04-29): Per `BLUEPRINT.md` (15.0/5.1/15.1–15.6), project status is tracked there as source of truth; latest work hardens model-list 503 resiliency, fixes relay-status API routing before SPA catch-all, removes recovered-gap error flashes, adds done/idle reconciliation retries for intermittent truncated assistant rendering, introduces forensic client-event logging (`/api/forensics/*` + `.forensics/client-events.ndjson`), adds SSE listener coverage for `status`, and enforces numeric event-id ordering in frontend SSE coalescing to prevent out-of-order chunk concatenation artifacts, with fullstack stream E2E (`scripts/e2e-fullstack-stream.mjs`) extended to validate forensic gap/order signals.
