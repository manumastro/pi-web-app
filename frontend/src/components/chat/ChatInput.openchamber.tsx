...
const ChatInputComponent: React.FC<ChatInputProps> = ({ onOpenSettings, scrollToBottom }) => {
        ...
        if (inputMode === 'normal' && normalizedCommand.startsWith('/')) {
            ...
            if (commandName === 'undo' && currentSessionId) {
                ...
                try {
                    ...
                    const { opencodeClient } = await import('./openchamber-stubs');
    ...
    React.useEffect(() => {
        ...
        void (async () => {
            try {
                const { getCurrentWebviewWindow } = await import('./openchamber-stubs');
                    ...
                    if (type === 'drop') {
                        ...
                        for (const path of paths) {
                            try {
                                ...
                                if (isTauriShell()) {
                                    const { invoke } = await import('./openchamber-stubs');
...
export const ChatInputOpenChamberCloned = React.memo(ChatInputComponent);

export const ChatInput = React.memo(function ChatInputAdapter(props: {
    text?: string;
    loading?: boolean;
    canSend?: boolean;
