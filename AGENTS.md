...
Current state (2026-05-03): Service now runs via interactive bash (`bash -ic`) so `.bashrc` matches CLI behavior, MiniMax direct auth works, and web model list is strict RPC-derived (no synthetic enabled-model placeholders).
