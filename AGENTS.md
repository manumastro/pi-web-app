...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, `/api/agent` retry storm, message-ID reconciliation, and added frontend no-op guards on session/directory setters to reduce React update-depth loops.
