...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, `/api/agent` retry storm, message-ID reconciliation + assistant stream IDs, and now materializes synthetic assistant messages on part-first stream events to avoid “working then empty response” UI turns.
