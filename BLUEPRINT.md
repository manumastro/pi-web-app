...
| Model listing + switching | **P0** | V1 | Low | ✅ Proven (session-scoped CLI wrapper; live `pi --mode rpc` capabilities filtered/ordered by CLI `enabledModels`) |
...
#### Done (2026-04-26)
- Added `docs/PI_RUNNER_ORCHESTRATOR_MIGRATION.md` as the one-shot migration plan for replacing the in-process runner backend with a CLI-wrapper runner/orchestrator architecture.
...
- Continued Pi Web UX parity with first product-navigation/chat polish: added a `Cmd/Ctrl+K` command palette for new-session/session/project/model commands and upgraded tool cards with explicit status pills plus copy input/output controls.
...
- Began strict Pi Web visual/UX refinement: replaced the visible app chrome with Pi Web logo/brand, relay-connected status in header/sidebar, dark neutral Pi Web tokens, CLI-wrapper header action cluster, active-session conic chase border, and a dock-panel shell opened by Files/Terminal/Git header buttons.
- Completed the first real workspace-panel pass: added `/api/workspace` endpoints for safe cwd-scoped file listing/reading, git branch/status/diff, and a scoped terminal command runner; replaced placeholder dock content with Files/Git/Terminal panels; restored the active-session Pi Web chase border for the selected session; and wired agent-driven session naming through `session_name` SSE events so the live title updates from the runner instead of the first user prompt.
...
- Completed CLI-wrapper alignment pass to avoid backend runner dependencies entirely: replaced remaining backend `@mariozechnerlocal types` type imports with local thinking-level types (`backend/src/types/thinking.ts`) so backend runtime/contracts stay CLI-wrapper focused.
- Replaced first-prompt auto session naming with agent-driven agent-driven naming: runner protocol now supports `session_name` events, runner-process maps `set_session_name` RPC tool executions into that event (and suppresses visible tool cards for that internal tool), orchestrator persists titles from runner events, and frontend consumes status metadata to update session titles live.
...
- Reload/tool-overlap follow-up hardening: lifecycle reducers now always receive the active directory when handling SSE payloads (so sync-store `session_status` stays aligned and avoids intermittent missing working indicators after reload), assistant-status computation falls back to the selected session status when sync-store status is temporarily unavailable, and tool/thinking events missing `messageId` are attached to the latest turn instead of rendering as overlapping orphan cards below the answer.
- Tool-label polish after tool completion: assistant-status tool detection now tracks unresolved tool-call/result pairs in the latest turn and prioritizes `Writing...` when assistant text is already streaming, preventing stale `Running web_search...` labels after tool execution has already completed.
- Working-placeholder reload layout polish: tightened `.working-placeholder-label` CSS (`display:inline-flex`, `position:static`, stable line-height/no-wrap) so `Writing...`/activity text no longer drifts to bottom-left while the spinner is active after a page reload.
- Reload streaming rehydration fix: when a session is still running but no assistant chunk is persisted yet, the synthetic streaming assistant placeholder now reuses the latest known turn `messageId` from conversation items (user/tool/thinking) instead of generating a detached random turn id, keeping in-progress `Writing...` feedback attached to the active tool turn after refresh instead of falling into a loose tail/orphan layout.
- Tool-call/result reload fidelity hardening: conversation mapping now tracks `toolCallId -> turnId` so persisted `tool_result` rows missing `messageId` still bind to the originating tool-call turn after refresh (instead of rendering as detached `result/success` orphans); live SSE `tool_result` handling applies the same fallback when `messageId` is absent.
- Added frontend regression coverage for running-refresh placeholder anchoring and missing-`messageId` tool-result turn attachment (`frontend/src/chatState.test.ts`, `frontend/src/components/chat/ConversationPanel.test.tsx`).
- Added frontend regression coverage in `frontend/src/hooks/useAssistantStatus.test.tsx` for post-tool streaming label behavior and pending-tool behavior.
- Verification after working-state fidelity hardening: `npm -ws --if-present test -- --run` (100 backend tests, 89 frontend tests), `npm -ws --if-present run lint`, and `npm -ws --if-present run build` are green.

#### Done (2026-04-29) (afternoon)
- Fixed sidebar CSS layout collapse bug (`sidebar-row` used `display:grid` which collapsed to `min-content` height in implicit rows, causing session items to stack at ~13px instead of ~38px): replaced with `display:flex` + `align-items:stretch` in `frontend/src/components/layout/Sidebar.css`, removed duplicate/conflicting definitions from `frontend/src/components/session/SidebarPanel.css` and `frontend/src/styles/piweb-shell.css`, and added `overflow:hidden` to `.sidebar-section`.
- Fixed session auto-rename not propagating to sidebar: the `session_name` SSE event was emitted by the orchestrator but never reached the frontend UI due to two bugs — (1) `backend/src/sse/manager.ts` `replayHistory()` returned early when `lastEventId` was undefined (fresh SSE subscriber), so the `session_name` event emitted before the SSE subscription was established was never replayed to the browser; fixed by always replaying all stored history events for fresh subscribers, and (2) `frontend/src/sync/event-reducer.ts` `reduceSessionLifecyclePayloads()` only handled `session_name` in an `else if` branch keyed on `finalPayload` (the last payload in the batch), but the replay batch is `[session_name, status, done, status]` so `finalPayload` is `status` and the `session_name` branch was never reached; fixed by moving `session_name` handling inside the for-loop so it processes each payload regardless of batch position.
- Added forensic `session_name` SSE event capture to `frontend/src/hooks/useSessionStream.ts` so `session_name` events are visible in `/api/forensics/tail`.
- Added session auto-rename E2E verification to `scripts/e2e-fullstack-stream.mjs` (checks sidebar title updates after first prompt for untitled sessions, skips rename check for subsequent prompts since session already has a title).
- Fixed sidebar duplicate CSS definitions: removed ~200 lines of duplicate `.directory-item`/`.session-item` rules from `frontend/src/components/session/SidebarPanel.css`, consolidated to single authoritative definitions in `frontend/src/styles/piweb-shell.css` + scoped overrides in `frontend/src/components/layout/Sidebar.css`.
- Build green, service restarted.

#### Done (2026-04-29)
- Model-capability resiliency hardening for field-reported intermittent model-picker failures: `backend/src/runner/orchestrator.ts` now treats `/api/models` capability fetch as best-effort by retrying `get_capabilities` without session scope when the session-scoped call fails (e.g. stale runner session after restart) and then falling back to cached/global model data instead of propagating transient runner errors as HTTP 503.
- Relay-status routing fix for field-reported false “Relay unavailable” UI states: `/api/relay/status` is now registered before the SPA static catch-all in `backend/src/server.ts`, preventing fallback HTML (`index.html`) responses on that API path.
- SSE gap-recovery UX hardening for field-reported transient error flashes: `frontend/src/sync/use-app-controller.ts` no longer surfaces recovered stream gaps as sticky chat errors (`setError`), and instead logs a warning plus recovery status while keeping explicit error state only for true recovery failures.
- Added fullstack browser-stream E2E guardrail script `scripts/e2e-fullstack-stream.mjs` (Playwright): validates prompt send, live stream activity, assistant render without reload, persisted transcript integrity, and fails on runtime/console errors or visible `Recovery stream gap` flashes.
- Verification after reliability fixes: `npm -w backend run -s test` is green (100 passed), `npm -w frontend run -s test` is green (92 passed), and `node scripts/e2e-fullstack-stream.mjs` is green.
- Assistant turn-binding hardening for intermittent duplicate/empty assistant bubbles: `frontend/src/sync/conversation.ts` now enforces strict `messageId` matching (no silent fallback on mismatched ids), rebinds first backend chunks onto optimistic placeholder assistants (including transient `Working`/`CLI idle` placeholder text), and ignores premature `done` events while placeholder-only assistant content is still present; paired reducer guardrails keep transport streaming active while placeholders are pending.
- Added regression coverage for strict turn binding and premature-done handling in `frontend/src/sync/conversation.test.ts` and kept stream lifecycle assertions in `frontend/src/sync/event-reducer.test.ts`; frontend suite now passes with expanded coverage (`npm -w frontend run -s test`, 98 passed).
- Added/iterated event-driven screenshot capture harness `scripts/e2e-continuous-screenshots.mjs` and preserved only bug-reference frames for forensic comparison in `screenshots/continuous/run-2026-04-29T12-42-29-643Z/0013-*.png` and `0016-*.png`.
...
- PWA narrow-width follow-up: enforced single-row mobile composer controls (`model | thinking | send/stop`) with explicit grid columns and standalone-mode overrides to prevent send button wrapping under iPhone PWA viewport quirks; final source-order lock in `frontend/src/index.css` keeps the send/stop button in the third grid column even when iOS PWA applies late viewport/layout quirks.
- Verification after mobile UX/cache hotfix: `npm run lint --workspace=frontend` and `npm run build --workspace=frontend` are green.
- Streaming/tooling hardening: runner-process now emits a fallback `done` when the blocking RPC prompt returns without an `agent_end`, preventing end-of-stream stuck UI; runner/orchestrator forwards status/usage metadata updates, assistant status surfaces token/context metadata when available, selected model context/max-token metadata is preserved from `/api/models` into the frontend, and consecutive tool bursts are collapsed into an expandable grouped tool card to avoid chat jank.
- CLI communication latency hardening: prompt send no longer performs a separate frontend model-sync preflight, backend emits `busy` status immediately after accepting the user turn, existing RPC sessions skip redundant start-session model/thinking/capability refreshes when unchanged, and runner-process acknowledges `send_input` as soon as it is handed to the CLI instead of holding the REST request until generation completes.
- Verification after streaming/tooling/latency hardening: test scripts now force `NODE_ENV=test`, fixing the frontend React production `React.act is not a function` test-environment failure, and the compact composer regression test was updated for the current minimal mobile controls. `npm test` is green (100 backend tests, 92 frontend tests), `npm run lint` is green, and `npm run build` is green.
...
- Mobile runtime detection now initializes root classes/CSS variables from the browser environment (`frontend/src/hooks/useMobileRuntime.ts` + `frontend/src/lib/mobileRuntime.ts`), so the existing mobile-safe-area/drawer/keyboard CSS actually activates on touch devices; header/content/sidebar/composer shells are wired to the corresponding safe-area utility classes and regression tests cover runtime + viewport metric calculation.
- Compact layouts now auto-collapse the sidebar on initial load and when crossing the desktop/mobile breakpoint (`frontend/src/App.tsx`), preventing the main chat container from starting squeezed by the project rail on phones while still keeping the sidebar available as a drawer via the header toggle.
- Fixed a mobile layout regression where the main chat column still rendered at the old sidebar width because the desktop `grid-template-columns` rule in `frontend/src/index.css` overrode the compact/mobile layout override from `styles/chat.css`; `index.css` now reapplies a single-column grid for compact/mobile shells so header/content span the full viewport width.
- Mobile shell/composer/sidebar behavior now moves materially closer to OpenChamber: `MainLayout` uses a dedicated compact drawer path with edge-swipe/drag gestures (`frontend/src/hooks/useMobileSidebarGesture.ts`), a compact mobile header variant (`frontend/src/components/layout/Header.tsx`), a mobile sidebar variant with dedicated drawer chrome/actions (`frontend/src/components/session/SidebarPanel.tsx`), and an OpenChamber-style compact composer flow (`frontend/src/components/chat/ComposerPanel.tsx`) with a mobile status strip plus bottom-sheet controls for model/thinking instead of exposing the full desktop control row inline; browser verification confirmed the compact header/content now span the full 390px viewport while the mobile drawer sits off-canvas until opened.
- Added frontend regression coverage for the new compact composer controls drawer (`frontend/src/components/ComposerPanel.test.tsx`) while keeping the full frontend suite green.
