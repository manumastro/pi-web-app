...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, SDK message/model metadata, and now emits live `message.updated` events with sortable user→assistant IDs so OpenChamber streaming/reload ordering matches Pi wrapper sessions.
