...
function applyServiceEnvironment(): void {
  ...
  if (process.env.PI_WEB_CWD && !process.env.PI_CWD) {
    process.env.PI_CWD = process.env.PI_WEB_CWD;
  }
  ...
  if (process.env.PI_WEB_MODEL && !process.env.PI_MODEL) {
    process.env.PI_MODEL = process.env.PI_WEB_MODEL;
  }
}
