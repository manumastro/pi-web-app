...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, `/api/agent` retry storm, message-ID reconciliation, frontend no-op guards on session/directory setters, and duplicate message-ID collisions (assistant now uses internal ID) that could trigger React update-depth loops.
