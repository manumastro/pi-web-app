...
2026-05-04: Fixed assistant text + thinking duplication root cause for snapshot-correction adapter events. Added orchestrator-level text correction detection (incoming text sharing >50% prefix with emitted content is emitted as `message.part.updated` replacement, not appended delta). Extended runner-process overlap normalization to thinking deltas via `emittedThinkingBuffer` (matching `emittedTextBuffer`), preventing cumulative thinking snapshots from duplicating. Added `replace` flag to `text_chunk` SSE schema/event-mapper, and per-event diagnostic logs in orchestrator. Backend tests pass (119/119). Frontend rebuilt and service restarted.
...
2026-05-04: Fixed broken chat-footer telemetry/controls UI. `SessionContextInfo` now reuses the existing OpenChamber `ContextUsageDisplay` and reads authoritative RPC token telemetry from `session-ui-store.getContextUsage(...)` (using model `limit.context` / `limit.output`) instead of the previously added local character-based token estimator. Also stabilized compact thinking-level selector rendering by removing nested tooltip+dropdown trigger composition, using a single dropdown trigger with title tooltip text, and adding resilient level-label fallback mapping. Then moved both context usage + thinking controls from chat timeline footer to the composer/model-controls area so they appear next to model selection as requested. Follow-up: wired RpcClient runtime context metadata through backend session mapping (`statusMetadata`/`statusMessage` in `toSdkSession`) so frontend can consume CLI-like `getSessionStats/getState` telemetry, and hardened right-sidebar/runtime null-safety in both ModelControls and session-tree rendering (`Array.isArray` guards for provider models, session message arrays, and `SessionNode.children`) to eliminate remaining `undefined.length` crashes.

2026-05-04: Added dynamic Pi CLI resolution in `runner-process/main.ts`: the system now auto-detects whether to use the NVM global Pi (`@mariozechner/pi-ai@0.72.1`, includes all models like `hy3-preview-free`) or the bundled Pi (`@mariozechner/pi-ai@0.67.68`, missing newer models). It compares version numbers and prefers the newer one. This fixed the intermittent `opencode/hy3-preview-free` failure where the model was not found in the registry. Also added `/api/system/info` endpoint exposing `piVersion` (backend) and a "Pi X.X.X" label in the AboutDialog (frontend), so users can see which Pi version is being used. The AboutDialog now shows both the OpenChamber version and the Pi version in use. **IMPORTANT**: Always use the latest `@mariozechner/pi-coding-agent` (global NVM install) — older bundled versions lack newer models and API adapters.

2026-05-04: Added backend model-traceability for turns and richer backend logging: `/session/:id/prompt_async` now logs requested/normalized model and dispatch model; runner orchestrator logs per-turn completion with the effective model; session messages persist optional per-message `model`, and SDK mapping prefers message-level model metadata so turns can reflect the actually used model. Also removed automatic prompt fallback: unavailable requested models now return explicit errors (no silent model switch), and frontend now always surfaces `session.error` as toast (including nested backend error payloads). Fixed model availability signaling bug in orchestrator model summaries by separating real runner-available keys from enabled-model visibility placeholders, so placeholder entries are no longer marked available and selected for dispatch. Reproduced CLI-vs-web divergence for `opencode/hy3-preview-free` (CLI `pi -p --model ...` succeeds while RPC turn failed with model-not-found) and hardened prompt flow by aligning model resolution to live `listModels(sessionId)` keys before `set_model`, with diagnostic logs that print the available model keys when `set_model` fails; async failures also persist assistant error messages for chat visibility. Fixed `opencode/hy3-preview-free` 400 error ("invalid parameter 'tools[0].function.name'") by setting the requested model on the session BEFORE `startSessionIfNeeded`, so the pi RPC process spawns with `--model` and picks the correct API provider (openai-completions), avoiding the anthropic-messages mismatch that caused SiliconFlow to reject the request.

2026-05-03: Reduced production lag/no-response pain by loading shell environment in the user systemd service startup (`source ~/.bashrc`) so provider API keys can be inherited by Pi runtime, lowering backend log level from debug→info, and adding orchestrator fallback that persists/emits assistant error text when Pi returns `stopReason:error` without text deltas (preventing silent empty assistant turns).

2026-05-03: Fixed theme persistence regression in web runtime by treating already-persisted local theme preference (`themeMode`/legacy theme keys) as a user override during startup, so late `openchamber:settings-synced` events no longer force dark mode over explicit light/dark choices after reload.

2026-05-03: Fixed twenty-eight production blockers after modular API rollout: (1) `GET /api/git/worktrees/bootstrap-status` now returns the typed contract expected by frontend (`{ status, error, updatedAt }`) instead of a custom `{ bootstrapped }` payload, which stopped runaway polling loops and unblocked message send paths waiting on bootstrap readiness; (2) `prompt_async` now handles stale client model keys more safely by selecting a valid fallback model from `runner.listModels()` when the requested model is not found; (3) restored frontend compatibility endpoint `POST /api/sessions/:sessionId/message-sent` (204 no-op), eliminating send-time 404 noise in browser console; (4) `/api/agent` now returns a default bootstrap agent instead of `[]`, stopping repeated agent reload storms on startup; (5) preserved frontend message reconciliation IDs by keeping local message records when merging Pi snapshots and exposing `messageId` as external API identity, which fixes cases where freshly sent user messages degraded to placeholder UI (`?`); (6) added frontend guardrails to avoid redundant state writes in `setCurrentSession` and `setDirectory`, reducing risk of React maximum update-depth loops in noisy event/selection paths; (7) fixed duplicate message IDs in `/api/session/:id/message` by using client `messageId` only for user messages and internal IDs for assistant messages, preventing user/assistant key collisions that could destabilize chat rendering (React #185); (8) separated assistant streaming/event message IDs from user prompt message IDs in the runner orchestrator so `thinking/tool/text/done` updates no longer target the optimistic user message key, reducing render-loop and part-merge instability during active turns; (9) added explicit client-error telemetry (`POST /api/client-error`) and wired ChatErrorBoundary to report component stacks in production, enabling deterministic tracing of remaining React update-depth loops; (10) fixed frontend sync handling for part-first streams by materializing a synthetic assistant message on `message.part.updated` when `message.updated` has not arrived yet, preventing turns that show “Working” briefly and then render no assistant output despite successful backend completion; (11) fixed empty-state send routing by creating a session on-demand when `currentSessionId` is null and rejecting empty `sessionId` sends in `routeMessage`, preventing silent no-op sends and unintended model/session fallback behavior on first message; (12) completed the OpenCode SDK message contract for Pi-backed sessions by returning user `agent/model` and assistant `providerID/modelID/path/tokens` metadata, plus backend default model settings/provider defaults, preventing OpenChamber from falling back to Gemma/unknown `?` rendering when displaying Pi wrapper sessions; (13) fixed live-stream compatibility by emitting OpenCode-style `message.updated` events for the optimistic user message and assistant turn before text parts, and by deriving assistant stream IDs as `${userMessageId}_assistant` so OpenChamber's id-sorted message arrays keep user→assistant order both live and after reload; (14) fixed live token rendering by initializing text/reasoning parts once and then emitting `message.part.delta` events, rather than repeatedly sending `message.part.updated` with only the latest chunk, which caused the UI to display only the final character/chunk and made streaming animation janky; (15) improved perceived OpenChamber streaming smoothness for bursty Pi RPC output by flushing SSE writes immediately, disabling socket buffering on SSE routes, and frame-smoothing only visible text deltas while leaving reasoning/non-text events unthrottled so hidden reasoning does not artificially delay the visible answer; (16) matched OpenChamber's pending-turn semantics by announcing an incomplete assistant `message.updated` immediately after the user message, then sending a final completed `message.updated` on `done`, so the Working placeholder transitions into the same assistant turn instead of disappearing abruptly before the stream appears; (17) made the Working/Retry placeholder visibly animated with robust staggered dot animation plus entry animation, preserving reduced-motion behavior while avoiding a static-looking placeholder during longer Pi startup gaps; (18) fixed tool-call fidelity by preserving tool names/state across live `tool_result` updates and by attaching persisted tool call/result records to the assistant message in SDK snapshots, so reloads show the same tool activity instead of hiding or partially rendering tool calls; (19) fixed final live role corruption after tool-using turns by ensuring final `done` → `message.updated` resolves only stored assistant messages, not earlier tool_call/tool_result records that share the same assistant external id, preventing the assistant answer from being reclassified/rendered as a user message; (20) fixed repeated "Add project directory" prompts on refresh by hydrating the local projects store from backend `project.list` before showing the first-run directory dialog, so existing backend projects are reattached automatically instead of being treated as an empty setup; (21) fixed false "No sessions yet" empty states after refresh by forcing directory alignment to the active hydrated project when local session cache is empty, ensuring sync bootstrap targets the correct project directory and loads existing sessions instead of showing a first-run sidebar; (22) added sidebar empty-state recovery that repopulates global session snapshot from directory-scoped `session.list` when startup data comes back empty, preventing persistent empty sidebar despite successful `/api/session` responses visible in network; (23) added a final fallback in sidebar project rendering: when project store hydration is empty, derive temporary project sections directly from loaded session directories so sessions remain visible instead of showing a false "No sessions yet" state; (24) fixed appearance settings reverting after reload by syncing visual controls (UI/code font, interface/terminal size, spacing density, input bar offset) to shared settings on change, preventing server settings sync from overwriting local choices with stale defaults on startup; (25) fixed theme mode reversion (e.g., light returning to dark) by adding immediate, non-debounced flush support for settings writes and using it for theme persistence, so `themeMode`/`themeVariant`/`useSystemTheme` updates land before reload and are not overwritten by stale server settings during startup sync; (26) restored OpenChamber compatibility for configuration reload actions by implementing `POST /api/config/reload` and further hardening explicit theme setters (`setTheme`/`setThemeMode`/`setSystemPreference`) to immediately persist next theme prefs, preventing stale dark defaults from reasserting after manual reload flows; (27) prevented late startup settings-sync events from overriding manual theme changes by ignoring incoming `openchamber:settings-synced` theme payloads after any user-driven theme action in-session, eliminating immediate light→dark snap-back behavior; (28) fixed no-op theme switching in web runtime caused by false-positive VSCode mode detection by requiring both `runtime.isVSCode === true` and `runtime.platform === 'vscode'` before applying VSCode theme locking, restoring normal light/dark selection behavior in standard web deployments. Backend/frontend tests are green and service restarted.
2026-05-03: Fixed empty web model picker/API provider list (`/api/config/providers` returning `[]`) by fixing runner startup env poisoning: backend env bootstrap had been parsing shell files and overwriting `PATH` with unresolved `$HOME/...:$PATH`, causing Pi RPC child launch to fail with `spawn node ENOENT` and `get_capabilities` to return no models. Removed backend env parsing fallback and aligned service startup to CLI semantics by running `ExecStart` with interactive bash (`bash -ic`), so `.bashrc` exports (including provider keys) are applied the same way as terminal CLI sessions; model discovery and MiniMax direct auth now work in web. Then aligned model visibility with CLI `/model` behavior by honoring enabled-model entries even when temporarily unresolved by RPC. Then hardened virtualized frontend render paths (`JsonTreeViewer`, `VirtualizedCodeBlock`, `MessageList`, `ChangesSection`) against React #185 by adding `useFlushSync: false` to all `useVirtualizer()` calls. The root cause is React 19's stricter `useSyncExternalStore` handling (issue #31730): `@tanstack/react-virtual` defaults to `flushSync()` inside its `onChange` callback during `useLayoutEffect`, which forces synchronous re-render cascades that collide with zustand v5 subscriptions, causing "Cannot update a component while rendering a different component". Setting `useFlushSync: false` defers to async re-renders, avoiding the nested-update detection without sacrificing correctness — virtualizer still renders correctly, just without synchronous scroll correction between measurement and paint.
