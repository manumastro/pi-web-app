...
- Verification after streaming/tooling hardening: `npm run lint --workspace=backend`, `npm run lint --workspace=frontend`, and targeted backend runner protocol/orchestrator tests are green; targeted frontend ConversationPanel tests currently hit the existing React production `React.act` test-environment issue before assertions.
