...
2026-05-03: Fixed empty web model picker/API provider list (`/api/config/providers` returning `[]`) by fixing runner startup env poisoning: backend env bootstrap had been parsing shell files and overwriting `PATH` with unresolved `$HOME/...:$PATH`, causing Pi RPC child launch to fail with `spawn node ENOENT` and `get_capabilities` to return no models. Removed backend env parsing fallback and aligned service startup to CLI semantics by running `ExecStart` with interactive bash (`bash -ic`), so `.bashrc` exports (including provider keys) are applied the same way as terminal CLI sessions; model discovery and MiniMax direct auth now work in web. Finally switched web model visibility back to strict RPC behavior (no synthetic placeholders for enabled-but-missing keys), so web now reflects only models actually returned by `RpcClient.getAvailableModels()`.
