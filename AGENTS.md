...
Current state (2026-05-03): Modular backend API is live with passing tests; fixed SDK message/model metadata, sortable user→assistant IDs, OpenCode delta streaming, and added SSE flushing plus text-only smoothing so bursty Pi output animates closer to OpenChamber.
