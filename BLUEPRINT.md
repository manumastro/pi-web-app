...
- Thinking blocks are now shown immediately on send, grouped with their matching assistant turn, and rendered above the assistant message, so reasoning stays visually attached to the reply even before backend text arrives. The optimistic conversation row comes from the shared chat store, and the frontend now re-syncs the active model to the backend on send so refresh state works without manual re-selection.
...
- **Build/test green (21 frontend tests, 73 backend tests)**, live `pi-web.service` on `0.0.0.0:3210`.
