...
- **OpenChamber-aligned frontend** with Tailwind CSS v4, Radix UI primitives, light theme with warm/beige palette (oklch-based), IBM Plex Sans/Mono fonts, 304px sidebar, and 56px header; session resumption now flows through an OpenChamber-style `frontend/src/sync/*` layer (`bootstrap`, `event-reducer`, `sync-context`, `live-aggregate`, `sessionActivity`) instead of only local transport state.
...
- **Build/test green (31 frontend tests, 78 backend tests)**, live `pi-web.service` on `0.0.0.0:3210`.
