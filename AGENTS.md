...
Current state (2026-05-04): Composer shows context usage + thinking near model controls; context now prefers RpcClient runtime metadata (`getSessionStats/getState` → `statusMetadata` on session via API mapper) with fallback to message tokens. Added additional null-safety hardening for right-sidebar/session-tree and model-controls data paths (`Array.isArray` guards on provider/session node/message collections) to eliminate remaining `undefined.length` crashes.
