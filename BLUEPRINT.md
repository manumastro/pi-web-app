...
2026-05-04: Fixed text/thinking cross-duplication. Some adapters emit thinking content in both `thinking_delta` and `text_delta` streams. Added `thinkingContent` accumulator to orchestrator's ActiveTurn; text deltas are now stripped of any prefix matching already-emitted thinking content, eliminating visual duplication of reasoning blocks inside assistant text. Also fixed runner-process thinking deduplication via `emitThinkingDelta` (same overlap normalization as text). Backend tests pass (119/119). Frontend rebuilt and service restarted.
