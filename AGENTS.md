...
Current state (2026-05-02): Modular backend API is live with passing tests; fixed bootstrap/model/send regressions, `/api/agent` retry storm, message-ID reconciliation, frontend no-op guards on session/directory setters, duplicate message-ID collisions, and now isolates assistant stream IDs from user message IDs to prevent render-loop instability.
