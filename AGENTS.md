...
Current state (2026-04-21): See `BLUEPRINT.md` (sections **15.0 Status Snapshot**, **5.1 Feature Matrix**, and phase checklists **15.1–15.6**) as the single up-to-date source of project status; latest significant change ports the mobile shell materially closer to OpenChamber with a dedicated compact header variant, off-canvas swipeable sidebar drawer + mobile sidebar chrome, and a compact composer that moves model/thinking controls into a bottom sheet/status-strip flow, while preserving the earlier browser-driven mobile runtime/safe-area activation, compact-layout sidebar auto-collapse, browser-verified full-width mobile grid fix, and the prior OpenChamber-style streaming/history parity plus modularized stylesheet split under `frontend/src/styles/*`.
